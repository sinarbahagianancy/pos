...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (method === "POST" && url === "/api/products/adjust-stock") {
      ...
      // fields, if provided, are still recorded for traceability but they
      // are NOT written to the product's procurement_history (ad-hoc
    ...
    if (method === "POST" && url === "/api/batch-input") {
      ...
      >;
      try {
        const { createBatchInputHandler } = await import("../src/server/batchInput.js");
        const result = await createBatchInputHandler(input);
        return res.status(201).json(result);
      } catch (err) {
