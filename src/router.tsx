...
const AppLayout = () => {
  ...
  type MenuItem = { id: string; label: string; path: string; icon: string };
  type MenuGroup = { id: string; label: string | null; items: MenuItem[] };
        ...
        <nav className="flex-1 px-4 overflow-y-auto custom-scrollbar">
          {menuStructure.map((group, groupIdx) => (
            <div key={group.id} className={groupIdx > 0 ? "mt-6" : ""}>
              {group.label && (
                <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest px-4 pb-2">
                  {group.label}
                </p>
              )}
