    ...
    <div className="space-y-6 lg:space-y-8 pb-24 lg:pb-10 max-w-full overflow-x-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">
            Jl. Kramat Gantung No. 63 • Real-time Business Analytics
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
        ...
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm group hover:border-indigo-200 transition-all">
          ...
          <p className="text-2xl font-black text-slate-900 tracking-tighter tabular-nums">
            {formatIDR(revenueThisMonth)}
          </p>
          <div className="mt-4 flex items-center space-x-1.5">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">
              of {formatIDR(monthlyTarget)} target
            </span>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-[32px] border border-slate-200 shadow-sm group hover:border-indigo-200 transition-all">
          ...
          <p
            className={`text-2xl font-black tracking-tighter tabular-nums ${lowStockItems.length > 0 ? "text-red-600" : "text-green-600"}`}
          >
        ...
        <div className="space-y-6 lg:space-y-8">
          <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <h2 className="font-black text-slate-900 uppercase tracking-tighter text-xs">
                Low Stock Products
              </h2>
                ...
                lowStockProducts.map((product) => (
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2 shrink-0 ml-3">
                      <span
                        className={`text-sm font-black tabular-nums ${product.stock === 0 ? "text-red-600" : product.stock <= 2 ? "text-amber-600" : "text-yellow-600"}`}
                      >
                        {product.stock}
                      </span>
                      <div
                        className={`w-2 h-2 rounded-full ${product.stock === 0 ? "bg-red-500" : product.stock <= 2 ? "bg-amber-500" : "bg-yellow-500"}`}
                      ></div>
                    </div>
                  </div>
                ))
        ...
        <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          ...
          <div className="p-4 space-y-2 flex-1 overflow-y-auto max-h-[350px]">
            {topProducts.map(([model, stats], idx) => (
              <div
                key={model}
                className="p-4 bg-white border border-slate-100 rounded-[28px] flex items-center justify-between group hover:border-indigo-100 transition-all"
              >
                <div className="flex items-center space-x-4 min-w-0">
                  <span
                    className={`text-lg font-black ${idx === 0 ? "text-amber-500" : idx === 1 ? "text-slate-400" : "text-slate-200"} transition-colors`}
                  >
                    0{idx + 1}
                  </span>
                  <div className="min-w-0">
                    <p className="text-sm font-black text-slate-900 uppercase tracking-tight truncate max-w-[180px]">
                      {model}
                    </p>
                ...
                <div className="text-right shrink-0 ml-2">
                  ...
                  <p className="text-[9px] text-green-600 font-black uppercase tracking-widest mt-0.5">
                    +{formatIDR(stats.profit)} profit
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="p-6 lg:p-8 space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-slate-50 p-5 rounded-[24px] border border-slate-100">
                ...
                <p className="text-lg sm:text-xl font-black text-slate-900 tabular-nums tracking-tighter leading-none break-words">
                  {formatIDR(inventoryCost)}
                </p>
              </div>
              <div className="bg-indigo-50 p-5 rounded-[24px] border border-indigo-100">
                <p className="text-indigo-400 text-[9px] font-black uppercase tracking-widest mb-2">
                  Retail Valuation
                </p>
                <p className="text-lg sm:text-xl font-black text-indigo-900 tabular-nums tracking-tighter leading-none break-words">
                  {formatIDR(retailValue)}
                </p>
              </div>
              <div className="bg-green-50 p-5 rounded-[24px] border border-green-100">
                <p className="text-green-600 text-[9px] font-black uppercase tracking-widest mb-2">
                  Potential Profit
                </p>
                <p className="text-lg sm:text-xl font-black text-green-700 tabular-nums tracking-tighter leading-none break-words">
                  {formatIDR(potentialProfit)}
                </p>
              </div>
              <div className="bg-amber-50 p-5 rounded-[24px] border border-amber-100">
                <p className="text-amber-600 text-[9px] font-black uppercase tracking-widest mb-2">
                  Total Products
                </p>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 px-2 py-1 rounded-lg uppercase tracking-widest">
                  {targetProgress.toFixed(0)}% Achieved
                </span>
              </div>
              <div className="relative h-5 bg-slate-50 rounded-full border border-slate-100 overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-[8px] font-black text-white uppercase tracking-widest drop-shadow-sm">
                    {formatIDR(revenueThisMonth)} / {formatIDR(monthlyTarget)}
                  </span>
                </div>
              </div>
            </div>
