...
const WarrantyTrackerView: React.FC<WarrantyTrackerProps> = ({
  sns,
  sales,
  claims,
  onAddClaim,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl lg:text-2xl font-black text-slate-900 tracking-tight uppercase">
            Pusat Layanan & Garansi
          </h1>
          <p className="text-sm text-slate-500 font-medium">
            Lacak status garansi unit terjual dan manajemen klaim servis.
          </p>
        </div>
      </div>
            ...
            <h2 className="font-black text-slate-900 uppercase text-xs tracking-widest mb-6 flex items-center space-x-2">
              ...
              >
                <path
                  ...
                  strokeWidth="2"
                  d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 17h.01M9 17h.01M9 14h.01M3 17h1m4 0h1m-1-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"
                />
              </svg>
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2">
                  ...
                  <button
                    ...
                    className="bg-indigo-600 text-white px-6 py-3 rounded-xl text-sm font-black hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 w-full sm:w-auto"
                  >
                    Cek
                  </button>
                </div>
              ...
              {foundItem && foundSale && (
                <div className="p-5 rounded-2xl border border-slate-100 space-y-4 bg-slate-50/50 animate-in fade-in slide-in-from-top-4 duration-300">
                  <div className="flex justify-between items-start gap-4">
                    ...
                    <span
                      className={`text-[9px] font-black px-2 py-1 rounded-lg uppercase whitespace-nowrap ${isUnderWarranty(foundItem.warrantyExpiry) ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="text-xs font-bold text-slate-700">
                        {formatDate(foundSale.timestamp)}
                      </p>
                    </div>
                    <div>
                  ...
                  {isUnderWarranty(foundItem.warrantyExpiry) ? (
                    <button
                      ...
                      className="w-full mt-4 bg-slate-900 text-white py-3 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-slate-800 transition-all shadow-md"
                    >
          ...
          <div className="bg-white rounded-[32px] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
            <div className="p-6 border-b border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <h2 className="font-black text-slate-900 uppercase tracking-tighter">
                Monitoring Antrean Servis
              </h2>
              <span className="text-[10px] text-indigo-600 font-black px-2 py-1 bg-indigo-50 rounded-full uppercase tracking-widest w-fit">
                {claims.length} Klaim Berjalan
              </span>
            </div>
                  ...
                  {claims.length > 0 ? (
                      ...
                      <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-5">
                          <div className="flex flex-col max-w-[200px] lg:max-w-none">
                            ...
                            <span className="text-xs text-slate-400 font-bold uppercase truncate">
                              {c.customerName}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-5 text-slate-500 font-bold text-[11px] whitespace-nowrap">
                    ...
                    <tr>
                      <td colSpan={4} className="p-20 text-center text-slate-300 italic">
                        Belum ada antrean perbaikan unit.
                      </td>
                    </tr>
                  )}
                </tbody>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[150] p-4 overflow-y-auto">
          <div className="bg-white rounded-[32px] shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100 animate-in zoom-in duration-200 my-auto">
            ...
            <div className="p-6 lg:p-8 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  ...
                  <p className="font-black text-slate-900 font-mono text-sm uppercase truncate">
                    {foundItem?.sn}
                  </p>
                </div>
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
            ...
            <div className="p-6 bg-slate-50 flex gap-4">
              <button
                onClick={() => setShowClaimModal(false)}
                className="flex-1 py-4 bg-white border border-slate-200 text-slate-700 font-black rounded-2xl text-xs uppercase tracking-widest transition-all"
              >
                Batal
              </button>
              <button
                onClick={submitClaim}
                disabled={!issue}
                className={`flex-1 py-4 rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg transition-all ${!issue ? "bg-slate-300 text-slate-400 cursor-not-allowed" : "bg-indigo-600 text-white shadow-indigo-100 hover:bg-indigo-700"}`}
              >
                Simpan Tiket
              </button>
            </div>
