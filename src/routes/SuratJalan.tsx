...
const SuratJalanView: React.FC<SuratJalanViewProps> = ({
  ...
  const handleSubmit = async (alsoPrint: boolean) => {
    ...
    if (formItems.some((it) => it.quantity < 1)) {
      showToast("All items must have quantity >= 1", "error");
      return;
    }
    // For SN products, sn is required
    for (const it of formItems) {
      const product = products.find((p) => p.id === it.productId);
      if (product?.hasSerialNumber && !it.sn) {
        showToast(`SN is required for ${product.model}`, "error");
        return;
      }
    }
                    ...
                    >
                      ...
                      {product?.hasSerialNumber ? (
                        ...
                        <input
                          ...
                          value={item.sn}
                          onChange={(e) => handleItemChange(idx, "sn", e.target.value)}
                          placeholder="SN (opsional untuk non-SN)"
                          className="col-span-4 px-3 py-2 border border-slate-200 rounded-lg text-sm font-mono"
                        />
                      )}
                      <button
