...
const LoginView: React.FC<LoginProps> = ({ staffList, onLogin }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (selectedStaff) {
      ...
      }, 800); // Fake loading delay for UX
    }
  };

  const admins = ['Nancy', 'Mami', 'Vita'];
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className={`w-full max-w-md bg-white rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden relative z-10 transition-all duration-700 ${isAnimating ? 'opacity-0 scale-95 translate-y-10' : 'opacity-100 scale-100 translate-y-0'}`}>
        ...
        <div className="p-8 lg:p-10 pt-0">
          <form onSubmit={handleLogin} className="space-y-6">
                ...
                >
                  {staffList.map(name => (
                    <option key={name} value={name}>
                      {name} {admins.includes(name) ? '(Admin)' : '(Staff)'}
                    </option>
                  ))}
                </select>
            ...
            <button 
              ...
              className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl text-xs uppercase tracking-widest hover:bg-slate-800 shadow-xl shadow-slate-900/10 active:scale-95 transition-all flex items-center justify-center space-x-2"
            >
              {isAnimating ? (
                <>
