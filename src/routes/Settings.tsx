  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-2">
        <div>
          <h1 className="text-xl lg:text-2xl font-black text-slate-900 uppercase tracking-tight">Konfigurasi Sistem</h1>
          <p className="text-sm text-slate-500 font-medium tracking-tight">Atur metadata toko, pajak, dan hak akses operasional.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
        <div className="bg-white p-6 lg:p-8 rounded-[32px] border border-slate-200 shadow-sm space-y-6">
          <h2 className="font-black text-slate-900 flex items-center uppercase text-sm tracking-tighter">
            <svg className="w-5 h-5 mr-3 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
            Profil Gerai Utama
          </h2>
          <div className="space-y-6">
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nama Toko Operasional</label>
              <input 
                type="text" 
                disabled={!isAdmin}
        ...
        {isAdmin ? (
          <div className="bg-white p-6 lg:p-8 rounded-[32px] border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Mata Uang Basis</label>
                <select 
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Nilai PPN Global (%)</label>
                <input 
                  type="number" 
                  ...
                  onChange={(e) => setLocalConfig({...localConfig, ppnRate: Number(e.target.value)})}
                />
                <p className="mt-3 text-[10px] text-slate-400 font-medium italic">Nilai default: 11% (UU HPP No. 7 Tahun 2021).</p>
              </div>
            </div>
          </div>
        ) : (
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[150] p-4">
          <div className="bg-white rounded-[32px] shadow-2xl max-w-md w-full overflow-hidden border border-slate-100 animate-in zoom-in duration-200">
            <div className="p-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <h2 className="text-lg font-black text-slate-900 uppercase tracking-tighter">Tambah Staff Baru</h2>
              <button onClick={() => setShowAddStaffModal(false)} className="text-slate-400 hover:text-slate-600"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
